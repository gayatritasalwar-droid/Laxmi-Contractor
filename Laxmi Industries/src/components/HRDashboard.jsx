...
const HRDashboard = ({ session, activeMenu, setActiveMenu }) => {
  ...
  const departmentList = [
    "-- Select --", "Accounts", "Data Entry Operator", "Development", "Drilling & Tapping & Eicher",
  ...
  const fetchAllEmployees = async () => {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        const mappedEmployees = data.map(w => ({ ...w, id: w._id, _id: w._id }));
        setAllEmployees(mappedEmployees);
        const pendingHRWorkers = mappedEmployees.filter(employee => employee.status === 'pending_hr');
        setWorkers(pendingHRWorkers);
        localStorage.setItem('laxmi_workers', JSON.stringify(mappedEmployees));
      }
    } catch (err) {
      ...
      <body>
        <div class="letter-container">
          <div class="letter-header">
            <div class="company-name">LAXMI CONTRACTOR</div>
          ...
          <div class="letter-body">
            ...
            <div class="content">
              ...
              <p>We are pleased to confirm your joining with <strong>LAXMI CONTRACTOR</strong> as <strong>${employee.designation || 'Employee'}</strong> in the <strong>${employee.department || 'General'}</strong> department.</p>
            ...
            <table class="details-table">
              ...
              <td><strong style="font-size:16px;color:#1e3a8a;">${employee.punchCode}</strong></td>
              </tr>
            ...
            <div class="content"><p>We welcome you to the LAXMI CONTRACTOR family and look forward to a long and mutually beneficial association.</p></div>
            <div class="signature-section">
              <div class="signature-box"><br/><br/><div class="signature-line"></div><div class="signature-title">Employee Signature</div></div>
              <div class="signature-box"><div class="signature-name">Laxmi S.Reddy</div><div class="signature-line"></div><div class="signature-title">For LAXMI CONTRACTOR</div><div class="signature-role">Authorized Signatory</div><div class="signature-designation">Managing Director</div></div>
            </div>
          </div>
        </div>
  ...
  const finalizeEmployee = async (employee, punchInput) => {
    ...
    if (String(expectedPunchCode).trim() !== String(punchInput).trim()) {
      setPunchError(`❌ Invalid punch code! Expected: ${expectedPunchCode}`);
      showToast(`Invalid punch code! Expected: ${expectedPunchCode}`, true);
      return false;
    }

    try {
      const response = await fetch(`${API_BASE_URL}/workers/finalize/hr/${employee._id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ finalizedBy: session.userName })
      });
      ...
      if (result.success) {
        ...
        showToast(`❌ Finalization failed: ${result.error}`, true);
        return false;
      }
    } catch (err) {
  ...
  const handleRegisterStaff = async () => {
    ...
    try {
      ...
      if (result.success || result._id) {
        showToast(`✅ Staff ${staffFormData.fullName} registered! Punch Code: ${punchCode}`);
  ...
  const updateWorkerInMongoDB = async (workerId, updatedData) => {
    try {
      const response = await fetch(`${API_BASE_URL}/workers/update/${workerId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const finalizedWorkers = allEmployees.filter(w => w.status === "finalized");
  ...
  const renderContent = () => {
                ...
                <tbody>
                  {filteredWorkers.map(w => (
                    <tr key={w._id || w.id}>
                      ...
                      <td>{w.designation}</td>
                      <td>
                        <code style={{ fontSize: '18px', fontWeight: 'bold', background: '#e0e7ff', padding: '4px 10px', borderRadius: '6px', color: '#1e3a8a' }}>
                          {w.punchCode || 'NO CODE'}
                        </code>
                      </td>
                      ...
                      <td>
                        <button 
                          ...
                          style={{ background: '#f59e0b', color: 'white', padding: '6px 12px', border: 'none', borderRadius: '4px', cursor: 'pointer' }}
                        >
                          Finalize
                        </button>
                      </td>
    ...
    if (activeMenu === 'staff_registration') {
      return (
        <div className="content-card">
          <div className="card-header">
          ...
          <div style={{ padding: '20px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
              <div className="form-group">
                ...
                <input type="text" name="fullName" value={staffFormData.fullName} onChange={handleStaffInputChange} required />
              </div>
              <div className="form-group">
                <label>Aadhar (12 Digit) *</label>
                <input type="text" name="aadhar" value={staffFormData.aadhar} onChange={handleStaffInputChange} maxLength="12" required />
              </div>
              <div className="form-group">
                <label>Aadhar File</label>
                <input type="file" onChange={(e) => setStaffFormData({...staffFormData, aadharFile: e.target.files[0]})} accept="image/*,application/pdf" />
              </div>
              <div className="form-group">
                <label>Phone (10 Digit) *</label>
                <input type="tel" name="phone" value={staffFormData.phone} onChange={handleStaffInputChange} maxLength="10" required />
              </div>
              <div className="form-group">
                <label>Date of Birth *</label>
                <input type="date" name="dateOfBirth" value={staffFormData.dateOfBirth} onChange={(e) => handleDobChange(e, 'staff')} required />
              </div>
              <div className="form-group">
                <label>Gender</label>
                <select name="gender" value={staffFormData.gender} onChange={handleStaffInputChange}>
                  <option>Male</option>
              ...
              <div className="form-group">
                ...
                <select name="department" value={staffFormData.department} onChange={handleStaffInputChange} required>
                  {departmentList.map(dept => <option key={dept} value={dept === "-- Select --" ? "" : dept}>{dept}</option>)}
                </select>
              </div>
              <div className="form-group">
                <label>Sub Department</label>
                <input type="text" name="subDepartment" value={staffFormData.subDepartment} onChange={handleStaffInputChange} placeholder="Optional" />
              </div>
              <div className="form-group">
                <label>Designation *</label>
                <select name="designation" value={staffFormData.designation} onChange={handleStaffInputChange} required>
                  {designationList.map(des => <option key={des} value={des === "-- Select --" ? "" : des}>{des}</option>)}
                </select>
              </div>
              <div className="form-group">
                <label>Monthly Salary (₹) *</label>
                <input type="number" name="monthlySalary" value={staffFormData.monthlySalary} onChange={handleStaffInputChange} required />
              </div>
              <div className="form-group">
                <label>Bond Years</label>
                <input type="number" name="bondYears" value={staffFormData.bondYears} onChange={handleStaffInputChange} />
              </div>
              ...
              <div className="form-group" style={{ gridColumn: 'span 2' }}>
                <label>Address</label>
                <textarea name="address" value={staffFormData.address} onChange={handleStaffInputChange} rows="2"></textarea>
              </div>
            </div>
            {ageAlert.show && <div style={{ fontSize: '12px', marginTop: '10px', color: ageAlert.isValid ? '#10b981' : '#dc2626' }}>{ageAlert.message}</div>}
            <div className="info-box">
              <i className="fas fa-info-circle"></i> Punch code will be auto-generated and Staff will go to Production → CEO → HR for finalization <strong>(Staff)</strong> tag will appear
            </div>
            <div className="modal-footer" style={{ marginTop: '20px', display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
              <button className="btn-secondary" onClick={() => setActiveMenu('dashboard')}>Cancel</button>
              <button className="btn-primary" onClick={handleRegisterStaff} style={{ background: '#8b5cf6' }}>Submit Staff</button>
            </div>
          </div>
        </div>
      );
    ...
    if (activeMenu === 'worker_registration') {
      setShowRegistrationModal(true);
                    ...
                    return (
                      <tr key={emp._id || emp.id}>
                        <td>
                          ...
                          {emp.userType === 'staff' && <span style={{ marginLeft: '8px', fontSize: '11px', background: '#8b5cf6', color: 'white', padding: '2px 6px', borderRadius: '10px' }}>Staff</span>}
                          <br/><small>ID: {emp.applicationId || emp.id}</small>
                        </td>
                        ...
                        <td>
                          <span style={{ background: statusBadge.color + '20', color: statusBadge.color, padding: '4px 8px', borderRadius: '12px', fontSize: '12px', fontWeight: '500' }}>
                            {statusBadge.text}
                          </span>
                        </td>
                        <td>
                          {emp.status === 'pending_hr' && (
                            <button 
                              className="btn-finalize" 
                              onClick={() => { 
                          ...
                          {emp.status === 'finalized' && (
                            <button className="btn-pdf-sm" onClick={() => generateJoiningLetterPDF(emp)} style={{ background: '#dc2626', color: 'white', padding: '4px 12px', border: 'none', borderRadius: 4 }}>
                              Letter
                            </button>
                          )}
                          <button 
  ...
  const [formData, setFormData] = useState({
    fullName: '', department: '', designation: '', workerType: 'Worker',
    perDayRate: '', otRate: '', aadhar: '', dateOfBirth: '', joiningDate: '', mobile: '', aadharFile: null
  });
    ...
    <div className="dashboard-layout">
      <div className="side-menu">
        ...
        <div className="menu-footer">
          <i className="fas fa-check-double"></i> HR Authority
        </div>
      </div>
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '650px' }}>
            ...
            <div className="modal-body" style={{ padding: '20px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                ...
                <div className="form-group"><label>Aadhar File</label><input type="file" onChange={(e) => setStaffFormData({...staffFormData, aadharFile: e.target.files[0]})} accept="image/*,application/pdf" /></div>
            ...
            <div className="modal-footer">
              <button className="btn-secondary" onClick={() => setShowStaffRegistrationModal(false)}>Cancel</button>
              <button className="btn-primary" onClick={handleRegisterStaff} style={{ background: '#8b5cf6' }}>Submit Staff</button>
            </div>
          </div>
        </div>
      ...
      {showFinalizeModal && selectedWorker && (
        <div className="modal-overlay" onClick={() => setShowFinalizeModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '550px' }}>
            ...
            <div className="modal-body" style={{ padding: '20px' }}>
              <div style={{ background: '#fef3c7', padding: '20px', borderRadius: '12px', textAlign: 'center', marginBottom: '20px', border: '2px solid #f59e0b' }}>
                <p style={{ marginBottom: '10px', fontSize: '14px', color: '#92400e' }}>📌 Enter This Punch Code to Finalize:</p>
                <p style={{ fontSize: '36px', fontWeight: 'bold', letterSpacing: '6px', fontFamily: 'monospace', background: 'white', padding: '10px', borderRadius: '8px', display: 'inline-block', margin: '0' }}>
                  {selectedWorker.punchCode || 'NO CODE'}
                </p>
              </div>
              ...
              <div className="form-group">
                ...
                <input 
                  ...
                  style={{ 
                    ...
                    fontSize: '18px', 
                    borderRadius: '8px', 
                    border: punchError ? '2px solid #dc2626' : '1px solid #ccc',
                    textAlign: 'center',
                    letterSpacing: '4px',
                ...
                />
                {punchError && <div className="error-message" style={{ color: '#dc2626', marginTop: '8px', fontSize: '13px', textAlign: 'center' }}>{punchError}</div>}
              </div>
            </div>
