...
const ContractorDashboard = ({ session, activeMenu, setActiveMenu }) => {
  ...
  const getStatusGroup = (worker) => {
    ...
    if (status === 'pending_production' || status === 'pending_hr' || status === 'pending_ceo' || status === 'pending' || status === 'pending_contractor')
      return 'pending';
    return 'other';
  };
  ...
  const fetchWorkers = async () => {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        ...
        const contractorName = session.userName || session.user?.name || session.loginId;
        ...
        console.log("My workers:", myWorkersFiltered);
  ...
  const StatCard = ({ title, value, icon, color, onClick }) => (
    <div className="stat-card-enhanced" style={{ borderLeftColor: color, cursor: 'pointer' }} onClick={onClick}>
  ...
  const EmployeeTable = () => {
    ...
    const getDisplayStage = (worker) => {
      ...
      if (status === 'pending_contractor') return 'Contractor Pending';
      return 'Pending';
    };
      ...
      <div className="employee-table-container">
        <div className="table-header">
          ...
          <button className="btn-primary-sm" onClick={() => setShowRegistrationModal(true)}>
            <i className="fas fa-plus"></i> Register Worker
          </button>
        </div>
                  ...
                  <td>
                    <div className="employee-info">
                      ...
                      <div className="employee-details">
                        ...
                        <small>ID: {worker.applicationId || worker._id?.slice(-6) || 'N/A'}</small>
                      </div>
                    </div>
                  </td>
                   <td>{worker.department || 'N/A'}</td>
                   <td>{worker.designation || 'Worker'}</td>
                   <td><code style={{ fontSize: '12px', background: '#f1f5f9', padding: '2px 6px', borderRadius: '4px' }}>{worker.punchCode || 'Pending'}</code></td>
              ...
              {filteredWorkers.length === 0 && !loading && (
                <tr>
                  <td colSpan="5" className="no-data">
                    ...
                    <p>No workers found. Click "Register Worker" to add.</p>
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <>
        <div className="welcome-banner">
          <div className="welcome-content">
            <h2><i className="fas fa-handshake"></i> LAXMI CONTRACTOR</h2>
            <p>Contractor Dashboard</p>
            <small>Welcome back, {session.userName || session.loginId}</small>
          </div>
        ...
        <div className="stats-grid-enhanced">
          <StatCard title="TOTAL REGISTERED" value={stats.total} icon="users" color="#3b82f6" onClick={() => setActiveMenu('my_list')} />
          <StatCard title="FINALIZED" value={stats.finalized} icon="check-circle" color="#10b981" onClick={() => { setSelectedStage('finalized'); setActiveMenu('my_list'); }} />
          <StatCard title="PENDING" value={stats.pending} icon="clock" color="#f59e0b" onClick={() => { setSelectedStage('pending'); setActiveMenu('my_list'); }} />
        </div>
        <EmployeeFilters />
        <EmployeeTable />
      </>
      ...
      <div className="side-menu">
        <div className="menu-header">
          ...
          <div>
            ...
            <p>{session.userName || session.loginId}</p>
          </div>
        </div>
        {menuItems.map(item => (
        ...
        <div className="menu-footer">
          <i className="fas fa-building"></i> LAXMI CONTRACTOR<br/>
