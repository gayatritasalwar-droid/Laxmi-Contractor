...
const WorkerRegistration = ({ onClose, onSuccess, session }) => {
  ...
  const checkDuplicateAadhar = async (aadharNumber) => {
    ...
    try {
      ...
      const isDuplicate = workers.some(w => w.aadhar === aadharNumber || w.aadharNumber === aadharNumber);
  ...
  const saveToBackend = async (workerData) => {
    try {
      ...
      return result.success || result._id ? true : false;
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const workerData = {
        ...
        punchCode: punchCode,
        contractorName: session?.userName || session?.loginId || 'caur',
        registeredBy: session?.userName || session?.loginId || 'caur',
        createdAt: new Date()
      };
      
      const saved = await saveToBackend(workerData);
      ...
      showToast('❌ Error registering worker: ' + err.message, true);
    } finally {
      setLoading(false);
    }
  };
