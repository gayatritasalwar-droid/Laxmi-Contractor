        ...
        <div className="modal-overlay" onClick={() => setShowApprovalModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '600px' }}>
            ...
            <div className="modal-body">
              <div className="info-section" style={{ background: '#f8fafc', padding: '15px', borderRadius: '8px', marginBottom: '20px' }}>
              ...
              <div style={{ background: '#f1f5f9', padding: '15px', borderRadius: '8px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}><span><strong>MONTHLY SALARY (₹)</strong></span><input type="text" value={rate ? `₹${parseInt(rate) * 26}` : '₹0'} readOnly style={{ width: '200px', background: '#e2e8f0', padding: '8px', borderRadius: '6px', textAlign: 'right', fontWeight: 'bold' }} /></div>
                <div style={{ fontSize: '12px', marginTop: '8px', textAlign: 'right' }}>Daily Rate: ₹{rate || 0} × 26 days = Monthly Salary</div>
                {applyESI && <div style={{ fontSize: '12px', marginTop: '8px', textAlign: 'right', color: '#dc2626' }}>ESI/PF Deduction: ₹{rate ? Math.floor((parseInt(rate) * 26 * esiPfPercentage) / 100) : 0}</div>}
              </div>
