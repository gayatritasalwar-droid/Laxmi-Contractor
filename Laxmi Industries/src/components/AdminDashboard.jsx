...
const AdminDashboard = ({ session, activeMenu, setActiveMenu }) => {
  ...
  const [workerStatusTab, setWorkerStatusTab] = useState('all'); // NEW: filter by status
  ...
  const fetchWorkersFromMongoDB = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        setWorkers(data);
        localStorage.setItem('laxmi_workers', JSON.stringify(data));
        return data;
      }
  ...
  const deleteWorkerFromMongoDB = async (workerId) => {
    try {
      const response = await fetch(`${API_BASE_URL}/workers/delete/${workerId}`, { method: 'DELETE' });
      const result = await response.json();
      return result.success;
    } catch (err) { return false; }
  };
  ...
  const fetchAdminUsers = async () => {
    try {
      ...
      localStorage.setItem('laxmi_admin_users', JSON.stringify(mapped));
    } catch (err) {
  ...
  const loadAllData = async () => {
    try {
      ...
      if (!mongoWorkers || mongoWorkers.length === 0) {
        const storedWorkers = localStorage.getItem('laxmi_workers');
        if (storedWorkers) setWorkers(JSON.parse(storedWorkers));
      }
      const storedContractors = localStorage.getItem('laxmi_contractors');
      if (storedContractors) {
        ...
        localStorage.setItem('laxmi_contractors', JSON.stringify(defaultContractors));
      }
  ...
  const addNewWorker = async () => {
    ...
    const worker = {
      ...
      appliedDate: new Date().toISOString().slice(0,10), registeredBy: 'admin', company: 'LAXMI CONTRACTOR'
    };
    const saved = await saveToMongoDB(worker);
    if (saved) {
      const updatedWorkers = [...workers, worker];
      setWorkers(updatedWorkers);
      localStorage.setItem('laxmi_workers', JSON.stringify(updatedWorkers));
  ...
  const deleteContractor = (id) => {
    if (window.confirm('⚠️ Delete contractor?')) {
      ...
      localStorage.setItem('laxmi_contractors', JSON.stringify(updated));
            ...
            <table className="data-table">
              ...
              <tbody>
                ...
                {filteredWorkers.length === 0 && (
                  <tr>
                    <td colSpan="9" style={{ textAlign: 'center', padding: '40px' }}>No workers found for this filter.</td>
                  </tr>
                )}
              </tbody>
            </table>
    ...
    <div className="dashboard-layout">
      <div className="side-menu">
        ...
        <div className="menu-footer">
          ...
          <small>LAXMI CONTRACTOR v2.0 | Enterprise Edition</small>
        </div>
      </div>
        ...
        <div className="modal-overlay" onClick={() => setShowManualAdminModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '500px' }}>
            ...
            <div className="modal-body">
              ...
              <div className="form-group"><label>Password (optional)</label><input type="text" value={manualAdmin.password} onChange={(e) => setManualAdmin({...manualAdmin, password: e.target.value})} placeholder="Default: admin123" /></div>
            </div>
            <div className="modal-footer">
